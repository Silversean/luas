...
function user_setup()
	...
	send_command('bind ^f10 gs c cycle ProcMode')
...
function job_state_change(stateField, newValue, oldValue)
    ...
    if stateField == 'ProcMode' then
        enable('main','sub')

        if newValue == 'On' and sets and sets.Proc then
            equip(sets.Proc)
        else
...
function is_proc_mode_active()
    return state and state.ProcMode and (state.ProcMode.value == 'On' or state.ProcMode.current == 'On')
end

function customize_melee_set(meleeSet)

    -- ProcMode override: force proc weapons/set before normal DamageMode and engaged logic.
    if is_proc_mode_active() and sets.Proc then
        enable('main','sub')
...
function customize_idle_set(idleSet)
    ...
    -- ProcMode override: force proc weapons/set while idle too.
    if is_proc_mode_active() and sets.Proc then
        enable('main','sub')
        idleSet = set_combine(idleSet, sets.Proc)
        return idleSet
    end
...
function get_state_value_for_table(name)
    ...
    if type(state[name].current) == 'function' then
        return tostring(state[name]:current())
    end
...
job_table_config = {
    ...
    rows = {
        ...
        {'^F10 Proc Mode', function() return get_state_value_for_table('ProcMode') end},
        {'^F6 Weapon Lock', function() return get_state_value_for_table('WeaponLockMode') end},
